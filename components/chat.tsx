...
const toolIcons: Record<string, React.ReactNode> = {
  searchForum: <SearchIcon className="h-3 w-3" />,
  browseForum: <FolderTreeIcon className="h-3 w-3" />,
  readForumPost: <FileTextIcon className="h-3 w-3" />,
  grepForum: <SearchIcon className="h-3 w-3" />,
          ...
          <div className="w-full max-w-2xl text-center space-y-6 md:space-y-12">
            <div className="space-y-3 md:space-y-4">
              <div className="flex flex-col items-center justify-center gap-4 animate-slide-up">
                ...
                <div className="flex items-center gap-3">
                  <h1 className="text-2xl sm:text-3xl md:text-5xl tracking-tight text-foreground font-[family-name:var(--font-geist-sans)] font-semibold">
                    Cursor Forum
                  </h1>
                  <div className="relative group">
                    <span className="text-[10px] text-muted-foreground/60 cursor-default">search</span>
                    <div className="absolute left-1/2 -translate-x-1/2 top-full mt-1 w-64 p-2 rounded-lg bg-popover border border-border shadow-md opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-150 z-50 text-left">
                      <p className="text-[10px] text-muted-foreground leading-relaxed">
                        Indexed discussions, feature requests, bug reports, and community knowledge from the Cursor forum.
                      </p>
                    </div>
                  </div>
                </div>
              ...
              <p className="text-muted-foreground text-sm md:text-base animate-slide-up px-2" style={{ animationDelay: '50ms' }}>
                Search the Cursor community forum — feature requests, troubleshooting, and tips. Powered by{" "}
                <a
              ...
              <form onSubmit={handleSubmit}>
                <div className="relative rounded-2xl bg-muted/50 dark:bg-muted/30 border border-border/50 shadow-sm hover:shadow-md focus-within:shadow-md focus-within:border-border transition-all duration-200">
                  ...
                  <textarea
                    ref={textareaRef}
                    name="prompt"
                    placeholder="How do I set up custom keybindings in Cursor?"
            ...
            <div className="flex flex-col gap-2 md:gap-3 text-xs md:text-sm animate-slide-up" style={{ animationDelay: '150ms' }}>
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-2 md:gap-3">
                <button
                  onClick={() => {
                    setInput("What are the most requested features for Cursor?");
                  }}
                ...
                >
                  &ldquo;Most requested features?&rdquo;
                </button>
                <button
                  onClick={() => {
                    setInput("How do I configure Cursor for Python development?");
                  }}
                  className="p-3 rounded-xl text-left text-muted-foreground hover:text-foreground active:bg-muted/70 hover:bg-muted/50 transition-colors"
                >
                  &ldquo;Python setup guide?&rdquo;
                </button>
                <button
                  onClick={() => {
                    setInput("What keyboard shortcuts are most useful in Cursor?");
                  }}
                  className="p-3 rounded-xl text-left text-muted-foreground hover:text-foreground active:bg-muted/70 hover:bg-muted/50 transition-colors"
                >
                  &ldquo;Best keyboard shortcuts?&rdquo;
                </button>
              </div>
            </div>
          </div>
                ...
                >
                  ...
                  {m.role === "assistant" && status !== "streaming" && (
                    ...
                    <div className="mt-3 pt-3 border-t border-border/40 flex items-start gap-2 text-xs text-muted-foreground/70">
                       ...
                       <span>
                         Answers come from indexed Cursor forum discussions via <a href="https://trynia.ai" target="_blank" rel="noopener noreferrer" className="font-medium text-primary hover:underline transition-all">Nia</a>.
                       </span>
                    </div>
                    </>
                  )}
